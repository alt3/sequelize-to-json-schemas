...
class JsonSchema7Strategy extends StrategyInterface {
  ...
  /**
   * Returns the "$comment" property (but only if manager option `hideComments` is false).
   *
   * @example
   * {
  ...
  getPropertyForHasOneAssociation(association) {
    ...
    return {
      [association]: {
        $ref: `#/definitions/${pluralized}`, // eslint-disable-line unicorn/prevent-abbreviations
      },
    };
  }
