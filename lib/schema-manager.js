...
class SchemaManager {
  ...
  _getRawAttributes() {
    ...
    if (_modelOptions.get(this).include.length > 0) {
      return _pick(attributes, _modelOptions.get(this).include);
    }
  ...
  _getModelContainer() {
    ...
    _assign(result, this._getPropertySchema()); // some schemas
    _assign(result, this._getPropertyId(this._getModelFilePath(_model.get(this).name))); // some schemas
    _assign(result, this._getModelPropertyTitle(_model.get(this))); // all schemas
    _assign(result, this._getModelPropertyDescription()); // all schemas but only if user passed the model option
  ...
  _getAttributeContainer(attributeName, attributeProperties) {
    ...
    _assign(result, this._getPropertyId(this._getAttributePath(attributeName))); // `id`: some schemas
    _assign(result, typeMapper.map(attributeName, attributeProperties, _strategy.get(this))); // `type`: all schemas, strategy-specific types
    _assign(result, this._getAttributePropertyTypeOverride(attributeName, attributeProperties)); // override the type generated by type-mapper if 'schema' exists
    _assign(
      result,
    ...
    _assign(result, this._getPropertyReadOrWriteOnly(attributeName, attributeProperties)); // `readOnly` or `writeOnly`: some schemas

    _assign(result, this._getAttributeExamples(attributeName, attributeProperties)); // `examples`: requires sequelize property jsonSchema.examples
  ...
  _getModelPropertyTitle(model) {
    ...
    if (!title) {
      title = this.constructor._capitalize(plural.singular(model.name));
    }
   ...
   * @returns {string} Capitalized string if a string was passed, otherwise returns ''
   */
