...
class SchemaManager {
  ...
  generate(model, strategy, options) {
    ...
    // add the model part to the response, following the order described at
    // https://json-schema.org/learn/getting-started-step-by-step.html#properties
    const result = {};

    _.assign(result, this._getModelPropertySchema()); // some schemas
    _.assign(result, this._getModelIdentifier()); // some schemas
    _.assign(result, this._getModelTitle()); // all schemas
    _.assign(result, this._getModelDescription()); // all schemas but only if user provided the generator() option
    _.assign(result, this.constructor._getModelType()); // all schema
  ...
  /**
   * Ensure constructor options are valid.
   *
   * @private
  ...
  _verifyOptions(options) {
    ...
    if (typeof options.relativePaths !== 'boolean') {
      throw new TypeError(
        "SchemaManager configuration option 'relativePath' not of type 'boolean'",
      );
    }
  ...
  /**
   * Ensure model options are valid.
   *
   * @private
   * @param {object} options Merged default and user provided options.
   * @returns {null}
  ...
  _getModelPath() {
    ...
    if (_options.get(this).relativePaths) {
      return `/${path}`;
    }
  ...
  /**
   * Returns the `schema` property for the model (if supported by the strategy).
   *
   * @private
   */
  _getModelPropertySchema() {
    const property = _strategy.get(this).getModelPropertySchema();
    if (!property) {
      return null;
    }
   ...
   */
  _getModelTitle() {
    const { title } = _modelOptions.get(this);
  ...
  /**
   * Returns the `description` property for the model (if user provided).
   *
   * @private
   */
