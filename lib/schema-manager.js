...
class SchemaManager {
   ...
   * @param {string} options.hideComments True to render attribute property 'comment', defaults to false
   */
  constructor(options) {
  ...
  _verifyOptions(options) {
    ...
    if (typeof options.hideComments !== 'boolean') {
      throw new TypeError("Model option 'hideComments' not of type 'boolean'");
    }
  ...
  _getAttributeContainer(attributeName, attributeProperties) {
    ...
    _.assign(
      result,
      this.constructor._getPropertyReadOrWriteOnly(attributeName, attributeProperties),
    ); // `readOnly` or `writeOnly`: some schemas
  ...
  _getAttributePropertyComment(attributeName, attributeProperties) {
    if (_options.get(this).hideComments === true) {
      return null;
    }

    if (!this.constructor._hasCustomAttributeProperty(attributeProperties, 'comment')) {
      return null;
    }

    const { comment } = attributeProperties.jsonSchema;
   ...
   * @returns {string}
   */
  static _getPropertyReadOrWriteOnly(attributeName, attributeProperties) {
    if (attributeProperties.jsonSchema === undefined) {
      return null;
    }

    const { readOnly, writeOnly } = attributeProperties.jsonSchema;

    if (readOnly === undefined && writeOnly === undefined) {
      return null;
    }
    ...
    if (writeOnly && typeof writeOnly !== 'boolean') {
      throw new TypeError(
