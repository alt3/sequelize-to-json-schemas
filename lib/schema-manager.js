...
class SchemaManager {
   ...
   * @param {boolean} options.renderIdProperty True will render the '$id' field for properties. Default to false
  ...
  _getPropertySchemaId(path) {
    return _strategy.get(this).getPropertySchemaId(path);
  }
   ...
   */
  _getPropertyId(path) {
    const { renderIdProperty } = _modelOptions.get(this);
